    ...
    public function displayListContent()
    {
        ...
        foreach ($this->_list as $index => $tr) {
            ...
            foreach ($this->fields_list as $key => $params) {
                ...
                } elseif (isset($params['egio_image'])) {
                    // item_id is the product id in a product image context, else it is the image id.
                    $item_id = $tr[$params['image_id']];
                    if ($params['egio_image'] != 'p' || Configuration::get('PS_LEGACY_IMAGES')) {
                        $path_to_image = _PS_IMG_DIR_ . $params['egio_image'] . '/' . $item_id . (isset($tr['id_image']) ? '-' . (int) $tr['id_image'] : '');
                    } else {
                        $path_to_image = _PS_IMG_DIR_ . $params['egio_image'] . '/' . Image::getImgFolderStatic($tr['id_image']) . (int) $tr['id_image'];
                    }
                    // dump($params);
                    // dump($tr);
                    // dump($path_to_image);
                    // die;
                    $this->_list[$index][$key] = ImageManager::thumbnail($path_to_image, $this->table . '_mini_' . $item_id . '_' . $this->context->shop->id, self::LIST_THUMBNAIL_SIZE);
